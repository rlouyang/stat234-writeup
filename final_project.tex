...
For \mc, this is the best we ever saw. Both \ab~and \mc~work well with DQN, lr=0.001, original architecture, frame skip 3 or 4, update frequency 1, no annealing, no regularization, Huber loss, about 500 episodes. \cp~doesn't train so well though. It seems to prefer a larger learning rate.

In general, \mc~was rather difficult to learn, as the final policy seems to depend heavily on the random initialization. With suboptimal hyperparameters, the episodes under a random policy can extend to tens of thousands of time steps, and all transitions entered into the replay memory have $r = -1$, so the learned policy can diverge. 
...
% key,mean,std,max,min,25,50,75,game,model,agent,n_trains,processing,target_update,frame_skip,update_frequency,batch_size,gamma,initial_learning_rate,annealing,optimizer,loss_function,weight_decay,layer_sizes
...
\begin{table}[!htbp]
  ...
  \caption{Top 15 parameter-tuned means per game between DDQN-GS and DQN-GS for 10,000 train iterations.}
...
\begin{table}[!htbp]
  ...
  \caption{Huber Loss, annealing, no target update, RMSProp, 50k}
  \label{tab:grid_search_v2_50k}
\end{table}

Target network set to false
...
\begin{table}[!htbp]
  ...
  \caption{DDQN-GS, Huber Loss, annealing, 0.1 weight decay for 100k iterations.}
...
\begin{table}[!htbp]
  ...
  \caption{Model comparison with final hyperparameters over 100,000 train iterations.}
