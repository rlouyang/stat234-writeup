...
By this point, we established that DDQCNNs tended to outperform DQCNNs, so we then explored hyperparameter configurations that yielded good performance over $100,000$ iterations. We then performed a grid search, varying the batch size and learning rate
varied batch size, learning rate
...
See Table \ref{tab:model_selection_ddqn_pca_4} for testing a conventional deep neural network \todo{something}
...
\begin{table}[!htbp]
  ...
  \caption{Results for model selection of network structure of DDQN-PCA, using Huber loss, learning rate annealing, and a $0.1$ weight decay for $100,000$ training iterations.}
