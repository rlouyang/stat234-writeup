...
\begin{itemize}
    ...
    \mathbb{R}$, the transition probabilities. After taking action $a$, the agent moves from $s_t$ to $s_{t+1}$ with probability $p(s_{t+1} |s_t,
...
{
    \subfloat[Sample \imagetype~image of the \cp~game screen. \label{subfig:CartPole_\imagetype}]{%
        \includegraphics[width=0.30\textwidth]{game_images/CartPole_\imagetype.pdf}
    }
    \hfill
    \subfloat[Sample \imagetype~image of the \ab~game screen. \label{subfig:Acrobot_\imagetype}]{%
        \includegraphics[width=0.30\textwidth]{game_images/Acrobot_\imagetype.pdf}
    }
    \hfill
    \subfloat[Sample PCA image of the \mc~game screen. \label{subfig:MountainCar_\imagetype}]{%
...
{
    \subfloat[Sample PCA image of the \cp~game screen for the \pca~model. \label{subfig:cartpole_\pca_image}]{%
        \includegraphics[width=0.30\textwidth]{game_images/CartPole_\pca.pdf}
    }
    \hfill
    \subfloat[Sample PCA image of the \ab~game screen for the \pca~model. \label{subfig:acrobot_\pca_image}]{%
        \includegraphics[width=0.30\textwidth]{game_images/Acrobot_\pca.pdf}
    }
    \hfill
    \subfloat[Sample PCA image of the \mc~game screen for the \pca~model. \label{subfig:mountaincar_\pca_image}]{%
...
As a baseline, we ran a random policy on each game, with results in Figures \ref{fig:CartPole_random}, \ref{fig:Acrobot_random}, and \ref{fig:MountainCar_random}. The average rewards and durations for the three games are available in Table \ref{tab:random_rewards}. With this initial goal in mind, we proceeded to test different model architectures and hyperparameter settings to achieve the best possible empirical results over all three games.

\foreach \game in {CartPole, Acrobot, MountainCar}
{
    \begin{figure}[!ht]
        \subfloat[Rewards for a random policy in the \texttt{\game}~game. \label{subfig:\game_NoTraining_Random_rewards}]{%
            \includegraphics[width=0.45\textwidth]{\game/\game_NoTraining_Random_rewards.pdf}
        }
        \hfill
        \subfloat[Durations for a random policy in the \texttt{\game}~game.\label{subfig:\game_NoTraining_Random_duraions}]{%
...
{   
    ...
    {
        \begin{figure}[!ht]
            \subfloat[Rewards for the \model~model in the \texttt{\game}~game. \label{subfig:\game_\model_EpsilonGreedy_rewards_cpu}]{%
                \includegraphics[width=0.45\textwidth]{\game/\game_\model_EpsilonGreedy_rewards_cpu.pdf}
            }
            \hfill
            \subfloat[Losses for the \model~model in the \texttt{\game}~game. \label{subfig:\game_\model_EpsilonGreedy_losses_cpu}]{%
                \includegraphics[width=0.45\textwidth]{\game/\game_\model_EpsilonGreedy_losses_cpu.pdf}
            }
            \caption{Rewards and losses for the \model~model in the \texttt{\game}~game.}
...
One thing not discussed in great detail was how we decided upon which parameters we chose at every step. In addition to empirical evaluations, we also built generalized linear models on our hyperparameters, treating them as categorical variables and predicting the mean. By examining the coefficients of the model this gave us a rough estimate of how different features were correlated with the mean.

An interesting problem that we encountered while testing different models was that there was little correlation between the loss function and the performance of the model. Indeed, more often than not, the loss function continuously shoot up as the simulations ran without ever converging toward an actual minimization. Typically, this indicates that we are underfitting our model, since the model is never able to minimize the loss; however, online discussions claim that this is not a problem in a reinforcement learning setting. \todo{citation needed}

During our modeling, we tried keeping track of sample Q values as the model trained. Over time, we expected the sample Q to increase as the value of one action over another became more apparent. While the Q-value did tend to increase for the sample, the values tended to explode to unreasonably large numbers. This seemed to be a sign that our weights were exploding in our neural network, but after clamping our nodes' values to be between $-1$ and $1$, we were unable to get more reasonable estimates of sample Q-values. 
...
We had severely limited computational power and time, so extending the paper using more varied learning rates and annealing schedules could offer better performance. Also computing using GPUs (which were unfortunately not available) rather than CPUs would have been a substantial time boost.

Adding Hindsight Experience Replay to \mc~would be a good strategy, since \mc~tended to get stuck in suboptimal positions for long periods of time. 
