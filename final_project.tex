...
With a strong DDQN-GS model selected as our new baseline, we then began to test the performance of PCA-based models. With \mc~and \ab~being solved problems for several of our neural networks but \cp~failing to ever reach especially large scores, we then focused our efforts on improving \cp. Due to the limited time and resources we had to test various models, we left tuning neural network architectures as our final step, taking the discovered hyperparameters up to this point for granted. We then tested the (D)DQN-PCA on a variety of more complex neural network designs. See Table \ref{tab:model_selection_DDQN_PCA_5} for the results from testing several feed-forward deep neural network architectures for the DDQN-PCA model over 20,000 training iterations.
...
\begin{table*}[!ht]
    ...
    \caption{Results from testing several feed-forward deep neural network architectures for the DDQN-PCA model, sorted by reward in descending order. All experiments used a Huber loss function, learning rate annealing, a batch size of $128$, a learning rate of $0.001$, and a $0.1$ weight decay for 20,000 training iterations.}
