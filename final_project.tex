...
\foreach \imagetype in {original, processed}
{
    \subfloat[Sample \imagetype~image of the \cp~game screen. \label{subfig:CartPole_\imagetype}]{%
        \includegraphics[width=0.32\textwidth]{game_images/CartPole_\imagetype.pdf}
    }
    \hfill
    \subfloat[Sample \imagetype~image of the \ab~game screen. \label{subfig:Acrobot_\imagetype}]{%
        \includegraphics[width=0.32\textwidth]{game_images/Acrobot_\imagetype.pdf}
    }
    \hfill
    \subfloat[Sample PCA image of the \mc~game screen. \label{subfig:MountainCar_\imagetype}]{%
        \includegraphics[width=0.32\textwidth]{game_images/MountainCar_\imagetype.pdf}
    }
}
...
Recent developments and additions to DQNs also suggest promising improvements to the general DQN framework as well as the use of PCA in reinforcement learning. For instance, adding Hindsight Experience Replay to \mc~would be a good strategy, since \mc~tends to get stuck in suboptimal policies, and choosing good transitions can significantly improve learning. 
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{python main.py -g CartPole-v0 -a Random -e 1000 --nreplay 10000} for a random policy
    \end{itemize}
\end{enumerate}
